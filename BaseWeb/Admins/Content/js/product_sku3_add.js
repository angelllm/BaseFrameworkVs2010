...
function fill(color_id) {
    ...
    var fill_content = "";
    $.each(__sku_param_array, function (param_ind, param_item) {
        //only one sku
        if (_sku_length == 1) {
            ...
            if (param_ind == 0) {
                $("#" + _append_id).append(fillChild(param_item));
            }
            else if (param_ind == 1) {
                $("#" + _append_id).find(".table-cell").append(fillChild(param_item));
            }
            else if (param_ind == 2) {
                fill_content = "";
                $.each($("#" + param_item + " input[type='checkbox']:checked"), function (checkbox_index, checkbox_item) {
                    var _checkbox_item_name = $(checkbox_item).next().text();
                    var _id = $(checkbox_item).parent().attr("id");
                    fill_content += _table
                                        .replace("{id}", "tbl_" + param_ind + "_" + _id)
                                        .replace("{class}", param_item + " tc_" + _id + " row3")
                                        .replace("{1}", "tbl_" + _id)
                                        .replace("{x}", "cell_" + _id)
                ...
                $("#" + _append_id).find(".table-cell").find(".table-cell").append(fill_content);
            }
...
function fillChild(param_item) {
    var fill_content = "";
    $.each($("#" + param_item + " input[type='checkbox']:checked"), function (checkbox_index, checkbox_item) {
        var _checkbox_item_name = $(checkbox_item).next().text();
        var _id = $(checkbox_item).parent().attr("id");
        fill_content += __table_temp
                        .replace("{id}", "tbl_" + _id)
                        .replace("{tc}", "tc_" + _id)
                        .replace("{class}", param_item)
                        .replace("{rows}", "row-" + param_item)
                        .replace("{x}", _checkbox_item_name)
                        .replace("{first}", "first_" + _id)
                        .replace("{cell}", "cell_" + _id);
...
$("#btnSet").live("click", function () {
    ...
    if ($("input[name='rblCount']:eq(0)").attr("checked") == "checked" && typeof ($("input[name='rblCount']").attr("disabled")) == "undefined") {
        _globaSet.parent().parent().parent().parent().parent().parent().parent().parent().parent().find(".txt-count").val(__cur_txt_count_val);
    }
    else if ($("input[name='rblCount']:eq(1)").attr("checked") == "checked" && typeof ($("input[name='rblCount']").attr("disabled")) == "undefined") {
        var set_area = _globaSet.parent().parent().parent().parent().parent().parent().parent().parent();
        var set_area_class = $(".tc_" + set_area.attr("id").replace("tbl_", ""));
        set_area_class.find(".txt-count").val(__cur_txt_count_val);
    }
    else if ($("input[name='rblCount']:eq(2)").attr("checked") == "checked" && typeof ($("input[name='rblCount']").attr("disabled")) == "undefined") {
        var set_area = _globaSet.parent().parent().parent().parent();
        var set_area_class = $(".tc_" + set_area.attr("id").replace("tbl_", ""));
        set_area_class.find(".txt-count").val(__cur_txt_count_val);
    }
    else if ($("input[name='rblCount']:eq(3)").attr("checked") == "checked" && typeof ($("input[name='rblCount']").attr("disabled")) == "undefined") {
        ...
        _globaSet.parent().parent().parent().parent().parent().parent().parent().parent().parent().find(".txt-price").val(__cur_txt_price_val);
    }
    ...
    else if ($("input[name='rblPrice']:eq(2)").attr("checked") == "checked" && typeof ($("input[name='rblPrice']").attr("disabled")) == "undefined") {
        var set_area = _globaSet.parent().parent().parent().parent();
        var set_area_class = $(".tc_" + set_area.attr("id").replace("tbl_", ""));
        set_area_class.find(".txt-price").val(__cur_txt_price_val);
    }
    else if ($("input[name='rblPrice']:eq(3)").attr("checked") == "checked" && typeof ($("input[name='rblPrice']").attr("disabled")) == "undefined") {
        var set_area_class = $(".tc_" + _globaSet.attr("id").replace("tbl_", ""));
        set_area_class.find(".txt-price").val(__cur_txt_price_val);
    }
...
$("#btnSave").click(function () {
    ...
    $.each($("#sku-color-size .rows"), function (rows_index, rows_item) {
        ...
        $.each(_rows_item_table_cell.find(".sku-area-1"), function (sk1_rows_index, sk1_rows_item) {
            var _sk1_rows_item = $(sk1_rows_item);
            var sk1_rows_item_table_first = _sk1_rows_item.find(".table-first:eq(0)");
            var sk1_rows_item_table_cell = _sk1_rows_item.find(".table-cell:eq(0)");
            var _sku_name = sk1_rows_item_table_first.text();
            var _sku_id = sk1_rows_item_table_first.attr("id");
            var _sku_data = _color_id + "." + _sku_id.replace("first_", "");
            _json += "{'sku_name':'" + _sku_name + "',";
            _json += "'tbl_id':'" + _sk1_rows_item.attr("id") + "',";
            _json += "'tbl_class':'" + _sk1_rows_item.attr("class") + "',";
            _json += "'sku_id':'" + _sku_id + "',";
            _json += "'sku_class':'" + sk1_rows_item_table_first.attr("class") + "',";
            _json += "'sku_cell_id':'" + sk1_rows_item_table_cell.attr("id") + "',";
            _json += "'sku_cell_class':'" + sk1_rows_item_table_cell.attr("id") + "',";
            _json += "'sku_area_name':'" + $(".sku-nav li:eq(2)").text() + "',";
            _json += "'sku_data':'" + _sku_data + "',sk2:[";

            $.each(sk1_rows_item_table_cell.find(".sku-area-2"), function (sk2_rows_index, sk2_rows_item) {
                var _sk2_rows_item = $(sk2_rows_item);
                var sk2_rows_item_table_first = _sk2_rows_item.find(".table-first:eq(0)");
                var sk2_rows_item_table_cell = _sk2_rows_item.find(".table-cell:eq(0)");
                var _sku2_name = sk2_rows_item_table_first.text();
                var _sku2_id = sk2_rows_item_table_first.attr("id");
                var _sku2_data = _sku_data + "." + _sku2_id.replace("first_", "");
                _json += "{'sku_name':'" + _sku2_name + "',";
                _json += "'tbl_id':'" + _sk2_rows_item.attr("id") + "',";
                _json += "'tbl_class':'" + _sk2_rows_item.attr("class") + "',";
                _json += "'sku_id':'" + _sku2_id + "',";
                _json += "'sku_class':'" + sk2_rows_item_table_first.attr("class") + "',";
                _json += "'sku_cell_id':'" + sk2_rows_item_table_cell.attr("id") + "',";
                _json += "'sku_cell_class':'" + sk2_rows_item_table_cell.attr("id") + "',";
                _json += "'sku_area_name':'" + $(".sku-nav li:eq(3)").text() + "',";
                _json += "'sku_data':'" + _sku2_data + "',sk3:[";

                $.each(sk2_rows_item_table_cell.find(".sku-area-3"), function (sk3_rows_index, sk3_rows_item) {

                    var _sk3_rows_item = $(sk3_rows_item);
                    var sk3_rows_item_table_first = _sk3_rows_item.find(".table-first:eq(0)");
                    var _price = _sk3_rows_item.find(".table-price .txt-price").val();
                    var _count = _sk3_rows_item.find(".table-count .txt-count").val();
                    var _sn = _sk3_rows_item.find(".table-sn .txt-sn").val();
                    var _sku3_name = sk3_rows_item_table_first.text();
                    var _sku3_id = _sk3_rows_item.attr("id");
                    var _sku3_data = _sku2_data + "." + _sku3_id.replace("tbl_", "");
                    _json += "{'sku_name':'" + _sku3_name + "',";
                    _json += "'tbl_id':'" + _sk3_rows_item.attr("id") + "',";
                    _json += "'tbl_class':'" + _sk3_rows_item.attr("class") + "',";
                    _json += "'sku_id':'" + _sku3_id + "',";
                    _json += "'sku_class':'" + sk3_rows_item_table_first.attr("class") + "',";
                    _json += "'price':'" + _price + "',";
                    _json += "'count':'" + _count + "',";
                    _json += "'sn':'" + _sn + "',";
                    _json += "'data':'" + _sku3_data + "',";
                    _json += "'sku_data':'" + _sku3_data + "." + _price + "." + _count + "'";
                    _json += "},";
                    _price_json += "'" + _sku3_data + "':[{'price':'" + _price + "','count':'" + _count + "'}],"
                });
