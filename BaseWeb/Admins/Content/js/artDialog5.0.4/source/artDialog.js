...
* Date: 2015-1-20
* (c) 2015 llm, http://www.llmztt.com
* Modify :
* 1.mined
* 2.maxed
* 3.jquery draggable
* 4.add to toolbar show and hide
* 5.shake win to be  show or hide
* 6.add show or hide width animate
...
artDialog.fn = artDialog.prototype = {
    ...
    _create: function (config) {
        ...
        dom.content.css({'padding':config.padding,"width":"100%","height":"100%","margin":"0 auto"});
        dom.res.css('cursor', config.resize=="True" ? 'se-resize' : 'auto');
       //图标
       var icon = config.icon,
		   iconBg = icon && (_isIE6 ? {png: 'icons/' + icon + '.png'}
		   : {background: 'url(\'/admins/content/js/artDialog5.0.4/skins/icons/' + icon + '.png\') no-repeat  0% 33%',"margin":"0 40px","padding":"33px 51px","color":"#fff"});
     
		dom.content.css(iconBg || {background: 'none'});
        ...
        //最小化
        jQuery(this.config.toolBar).bind("click",function(){
            if (that.config.mined || that.config.mininged )  that.min();
            else{
                if ( artDialog.focus != that) that.zIndex();
                else that.min(); 
            }
            return that;
        });
    ...
    */
    ...
    orgin: function (x,y) {
        ...
        jQuery(wrap).animate({left:x,top:y});
    ...
    */
    ...
    animate_size: function (width, height) {
        ...
        if (width=="0px" && height=="0px" ) {
            jQuery(this.dom.main[0]).animate({width:width,height:height},function(){that.hidden();});
        }else{ 
            jQuery(this.dom.main[0]).animate({width:width,height:height},jQuery.noop());
        }
    ...
    */
    ...
    mining: function () {
         var that = this;
         var main = this.dom.main[0].style;
       ...
       if (this.config.mininged) {
            if (!this.config.mined) { 
                this.config.mininged = false;

                $width =that.config.minwidth;
                $height=that.config.minheight;
                $x=that.config.minx;
                $y=that.config.miny; 
                this.visible();
                //this.zIndex().visible();
             ...
             if (!this.config.mined) { 
                //最小化
                this.config.minwidth = main.width;
                this.config.minheight = main.height;
                this.config.minx = style.left;
                this.config.miny = style.top;
                var toolbar = jQuery(this.config.toolBar);
                $width = 0;
                $height= 0;
                $x=toolbar.offset().left;
                $y=toolbar.offset().top;  
                this.config.mininged = true; 
          }
       }

       return this.animate_size($width,$height).orgin($x,$y);
    },
    /** 最小化 */
    min: function () {
         var that = this;
        ...
        if (this.config.mined || this.config.mininged) {
          
            this.config.mined = false;
            this.config.mininged = false;
            $width =that.config.minwidth;
            $height=that.config.minheight;
            $x=that.config.minx;
    ...
    max: function () { 
     ...
     var style = this.dom.wrap[0].style;
     var main = this.dom.main[0].style;
     if (this.config.maxed) {
            //缩小
            this.config.maxed = false;
            _w = this.config.changwidth;
            _h = this.config.changheight;
            _x = this.config.changx;
            _y = this.config.changy;  
            return this.animate_size(_w,_h ).orgin(_x,_y);

        } else{
            _w =document.documentElement.clientWidth;
            _h =document.documentElement.clientHeight - 90;
            _x = "0px";
            _y = "0px";
            if (this.config.mined) { 
                this.config.changwidth = this.config.minwidth;
                this.config.changheight =this.config.minheight;
                this.config.changx =this.config.minx;
                this.config.changy = this.config.miny;
            }
            else{
                this.config.changwidth = main.width;
                this.config.changheight = main.height;
                this.config.changx = style.left;
                this.config.changy = style.top;
            } 
            this.config.maxed = true;
            return this.animate_size(_w,_h ).orgin(_x,_y);
      }
    ...
    */
    ...
    _addEvent: function () {
        ...
        .bind('mousedown', function () {
            that.zIndex();
            GLOBAOBJ = that.config.id;
        })
        .bind("dblclick",function(event){
            var target = event.target;
            if (target === dom.title[0] && that.config.isCanMax) {
                that.max();
                return false;
             }
        });
...
$(document).bind('keydown', function (event) {
    ...
    //f2
    if (keyCode===113) {
       if (jQuery(".icon-hover").length > 0) {
            jQuery(".icon-hover").find("input").show().select().prev().hide();
       }
       return false;
    }
    ...
    var flag = api.config.isCanClose; 
    // ESC
    if (flag) {
    ...
    //keyCode === 27 && api._click('cancel') && flag;
});
...
+                                       '<a class="d-min" title="{minValue}">-</a>'
+                                       '<a class="d-max" title="{maxlValue}">+</a>'
+										'<a class="d-close" title="{cancelValue}">×</a>'
+									'</div>'
+								'</td>'
+							'</tr>'
+							'<tr>'
...
+				'<td class="d-sw"></td>'
+				'<td class="d-s"></td>'
+				'<td class="d-se"><div style="position:relative"><i class="d-res" style="position:absolute;right:1px;top:-14px;width: 16px;height: 16px;background-image: url(/Admins/Content/images/ui-icons_222222_256x240.png);background-position: -64px -224px;"></i></div></td>'
+			'</tr>'
+		'</tbody>'
+	'</table>'
+'</div>';
