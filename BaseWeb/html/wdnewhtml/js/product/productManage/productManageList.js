function gotoPage(page,totalPage,pageSize){
	...
	$("#form_id").attr('action',projectPath+"/productManage/list.htm").submit();
...
function up_product(id){
	if(confirm("确定要上架吗?")){
		$("#productId").attr('name','productId').val(id);
		$("#form_id").attr('action',projectPath+"/productManage/up_product.htm").submit();
	}	
}
...
function delete_product(id,s){
	 $(".model").hide();
		$(s).prev("textarea").attr('name','reason').val();
		$("#productId").attr('name','productId').val(id);
		$("#form_id").attr('action',projectPath+"/productManage/delete_product.htm").submit();
